...
class TxTriShape : public Shape {
    ...
    void make_shader() {
        fTexShader = GCreateBitmapShader(fBM, GMatrix(), (GTileMode)fTile);
    }
...
public:
    ...
    void onDraw(GCanvas* canvas, const GPaint&) override {
        auto draw_tri = [](GCanvas* canvas, const GPoint pts[3], const GColor color) {
            ...
            const float r = 3;
            for (int i = 0; i < 3; ++i) {
                canvas->drawRect(GRect::LTRB(pts[i].x - r, pts[i].y - r,
                                             pts[i].x + r, pts[i].y + r), paint);
