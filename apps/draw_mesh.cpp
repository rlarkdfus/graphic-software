...
public:
    ...
    std::unique_ptr<GShader> bmShader(const GBitmap& bm) const {
        ...
        return GCreateBitmapShader(bm, mx, GTileMode::kRepeat);
    }
       ...
       //     show_quad_lines(canvas, fPts, level);
        ...
        if (fUseEProc) {
            ...
            for (GPoint p : fOffCurve) {
                canvas->drawRect(GRect::XYWH(p.x - 4, p.y - 4, 8, 8), pn0);
                canvas->drawRect(GRect::XYWH(p.x - 2, p.y - 2, 4, 4), pn1);
            }
        }
    }
    ...
    GClick* findClick(GPoint pt, GWindow* wind) override {
        for (int i = 0; i < 4; ++i) {
            if (mesh_hit_test(fPts[i].x, fPts[i].y, pt.x, pt.y)) {
                fCornerIndex = i;
        ...
        if (fUseEProc) {
            for (GPoint& oc : fOffCurve) {
                if (mesh_hit_test(oc.x, oc.y, pt.x, pt.y)) {
                    GPoint* ocPtr = &oc;
